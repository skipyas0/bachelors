...
In experiments, this prompt is successful in incentivizing parseable outputs adhering to the \texttt{Signature} specifications.
...
\begin{enumerate}
    \item \textbf{Loss of generality}: When using task samples $(q, g) \in \mathcal{D}$, the model $\mathcal{M}_{\text{optim}}$ might focus on a single query $q$ and thus fail to generate general instructions.
