...
Current state-of-the-art use the Transformer\cite{vaswani2023attentionneed} architecture and its derivatives. 
...
LLMs predict the next token in a sequence of tokens. Token can represent a letter or a word, but current models utilize sub-word tokens, similar in length to syllables.
The meaning of these tokens can be infered from relative position to other tokens in training text and encoded into vector form using an algorithm like Word2Vec\cite{mikolov2013efficientestimationwordrepresentations}.
...
The input sequence is generally called a \textit{prompt}. The model recursively generates next tokens, forming an output sequence, which we will also call a \textit{completion}.
